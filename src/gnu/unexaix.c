...
#define XCOFF
#define COFF
#ifndef NO_REMAP			/* SWI-Prolog conditions */
#define NO_REMAP
#endif
...
#ifndef emacs
#ifndef PERROR				/* SWI-Prolog defined */
     ...
     unsigned data_start, bss_start, entry_address;
{
  ...
  if (make_hdr (new,a_out,s_out,data_start,bss_start,entry_address,a_name,s_name,new_name) < 0
      || copy_text_and_data (new) < 0
      || copy_sym (new, s_out, a_name, new_name) < 0
#ifdef COFF; 
      || adjust_lnnoptrs (new, s_out, new_name) < 0
#endif
#ifdef XCOFF; 
      || unrelocate_symbols (new, s_out, s_name, new_name) < 0
#endif
      )
    {
      close (new);
     ...
     int new;
{
  ...
  Putf("data: %ld ", end - ptr);
  DEBUG(0, Putf("(0x%x-0x%x) ", ptr, end));
  DEBUG(0, Putf("&_data = 0x%x ", &_data));
  Putf("... "); pl_flush();
  write_segment (new, ptr, end);
 ...
 */
static int
copy_sym (new, s_out, s_name, new_name)
     int new, s_out;
     char *s_name, *new_name;
{
  char page[1024];
  int n;
...
unrelocate_symbols (new, s_out, s_name, new_name)
     int new, s_out;
     char *s_name, *new_name;
{
  register int i;
  ...
  Putf("rearranging ... "); pl_flush();
  lseek (s_out, orig_load_scnptr, 0);
  if (read (s_out, &ldhdr, sizeof (ldhdr)) != sizeof (ldhdr))
    {
      PERROR (new_name);
    }
...
#define SYMNDX_BSS	2
  ...
  for (i = 0; i < ldhdr.l_nreloc; i++, l--, ldrel++)
	...
	if (read (s_out, ldrel_buf, l * LDRELSZ) != l * LDRELSZ)
	  {
	    PERROR (s_name);
	  }
	ldrel = ldrel_buf;
      ...
      if (ldrel->l_rsecnm == f_ohdr.o_sndata)
	{
	  ...
	  lseek (s_out, orig_data_scnptr + ldrel->l_vaddr, 0);

	  if (read (s_out, (void *) &orig_int, sizeof (orig_int)) != sizeof (orig_int))
	    {
	      PERROR (s_name);
	    }
